...
```toml
name = "acme-health-alert"   # must be globally unique across Cloudflare
```
...
npx wrangler deploy
# Output:
#   Published acme-health-alert (X.XX sec)
#   https://acme-health-alert.<account-subdomain>.workers.dev
```
...
curl -i https://acme-health-alert.<sub>.workers.dev/check

# With token — expect 200 + {"checked":1,"failed":0|"failed":1}
curl -X POST -H "Authorization: Bearer $CHECK_TOKEN" \
  https://acme-health-alert.<sub>.workers.dev/check | jq
```
