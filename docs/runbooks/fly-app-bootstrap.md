...
Pick a slug matching `<client>-member` (kebab-case, ≤ 30 chars). Example for example.com: `acme-member`.
...
Verify: `fly apps list --org acme-member` shows both apps.
...
Run the same block with `-a acme-production` and live Stripe keys (`sk_live_…` + the production webhook signing secret).
...
```sh
fly secrets unset -a acme-staging \
...
Then trigger a redeploy (push to main, or `fly deploy -a acme-staging --remote-only`).

Verify: `fly secrets list -a acme-staging | grep STRIPE_PRICE` shows only `STRIPE_PRICE_1`, `STRIPE_PRICE_2`, `STRIPE_PRICE_1_RENEWAL`, `STRIPE_PRICE_2_RENEWAL`.

## 6. Rollback a single secret

```sh
fly secrets unset STRIPE_PRICE_1 -a acme-staging
fly secrets set   STRIPE_PRICE_1=price_new -a acme-staging
```
