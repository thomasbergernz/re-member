...
describe("google-sheets", () => {
  ...
  describe("appendCheckoutLog", () => {
    ...
    it("formats amountPaid in cents as $ with two decimal places", async () => {
      const appendCheckoutLog = await getAppendCheckoutLog();
      ...
      expect(call.requestBody.values[0][6]).toBe("$149.99");
    });
    ...
    it("succeeds with zero amountPaid", async () => {
      ...
      const call = mockAppend.mock.calls[0][0];
      expect(call.requestBody.values[0][6]).toBe("$0.00");
    });
