...
const SHEET_HEADERS = [
  ...
  "declaration_scope",        // Z
  "declaration_member_services",  // AA
  ...
  "doc_planning_cert_count",  // AL
  "doc_specialist_training_count", // AM
...
export async function createApplicantRow(
  ...
  const row = [
    ...
    0, // doc_planning_cert_count // AL
    0, // doc_specialist_training_count // AM
...
export async function updateApplicantFormData(
  ...
  data: {
    ...
    declarationMemberServices?: string;
  ...
  const colMap: Record<string, string> = {
    ...
    AA: data.declarationMemberServices ?? "",
...
export interface ApplicantInfo {
  ...
  docPlanningCertCount: number;
  docSpecialistTrainingCount: number;
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    if (rowToken === token) {
      ...
      return {
        ...
        declarationMemberServices: row[26] ?? "",
        ...
        docPlanningCertCount: Number(row[37]) || 0,
        docSpecialistTrainingCount: Number(row[38]) || 0,
...
export async function getUploadStatus(
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    if (row[0] === applicantId) {
      ...
      docs.planning_cert = Number(row[37]) || 0;
      docs.specialist_training = Number(row[38]) || 0;
...
function isFormComplete(applicant: ApplicantInfo): boolean {
  ...
  const declarations = [
    ...
    applicant.declarationMemberServices,
...
export async function validateCompletion(applicantId: string): Promise<boolean> {
  ...
  const requiredCounts = [
    ...
    applicant.docPlanningCertCount,
    applicant.docSpecialistTrainingCount,
