 ...
 *
 * The blueprint ships configured for a New Zealand org: NZD currency, the
 * Pacific/Auckland timezone, and a 1 July membership-year anchor (renewals
 * and deferred subscriptions align to the next 1 July). These values were
 * previously hardcoded across the checkout, pricing, sheet-logging and email
 * modules. Centralised here so a non-NZ instance can change them in one place.
 *
 * See docs/CUSTOMIZE.md → "Localisation constants" before deploying for a
 * different region. Note that Stripe Prices must also be created in the
 * matching currency, or the currency guard in stripe-products.ts will reject
 * them and /api/health will report "degraded".
 */

/** ISO 4217 currency code, lower-case (Stripe's convention for `currency`). */
export const CURRENCY = "nzd";

/** Symbol prefixed to formatted money amounts, e.g. "NZ$". */
export const CURRENCY_SYMBOL = "NZ$";
...
/** Format an integer cent amount as a display string, e.g. 7500 → "NZ$75.00". */
