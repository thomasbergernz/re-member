...
- **REQ-AA-008** Declarations: 8 checkboxes (accuracy, ethics, scope, member services, interview, professional development, criminal check, meetings) + `signed_at` ISO timestamp → columns X–AF.
- **REQ-AA-009** Document Upload: 6 required categories (training, ethics, criminal, planning_cert, specialist_training, fundamentals) + 1 optional (insurance). See spec `004`.
