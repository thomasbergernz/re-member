...
- **REQ-FS-008** Field types: `text`, `textarea`, `select`, `checkbox`, `radio`, `date`, `email`, `tel`, `number`, `repeatable`, `group`, `grid`. (`file` is handled outside the FieldType union via `schema.uploads` + the upload API, per REQ-FS-012; `signature` and `hidden` are reserved for future need and are NOT implemented — backfill correction 2026-07-03.)
